...
static void benchmark_function(size_t size, size_t q_size, int64_t iterations,
                               const std::function<float(void)> &func) {
...
int main(int argc, char *argv[]) {
    ...
    float *test_data1 = (float *)align_with_offset(test_data1_v.data(),
                                                   params.alignment_offset);
    float *test_data2 = (float *)align_with_offset(test_data2_v.data(),
                                                   params.alignment_offset);
    float *test_q1 =
        ...
        if (qfns.from_float && qfns.to_float) {
            ...
            if (params.op_quantize_row_q_reference) {
                ...
                for (size_t size : params.test_sizes) {
                    ...
                    benchmark_function(size, quantized_size, iterations,
                                       quantize_fn);
                }
                printf("\n");
            }
            ...
            if (params.op_quantize_row_q_dot) {
                ...
                for (size_t size : params.test_sizes) {
                    ...
                    auto quantize_fn = [&](void) -> float {
                        auto vdot =
                            ggml_internal_get_type_traits(qfns.vec_dot_type);
