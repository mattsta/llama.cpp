...
int main() {
    ...
    for (size_t i = 0; i < 24; ++i) {
        uint32_t *cp =
            new uint32_t[2]; // dynamically allocate memory for code_point
        cp[0] = 37 + i;
        cp[1] = 0;
    ...
    std::vector<std::vector<std::pair<uint32_t, uint16_t>>> expected_reject = {
        ...
        {
            {0, 37},  {1, 38},  {2, 39},  {4, 41},  {5, 42},  {6, 43},
            {7, 44},  {8, 45},  {9, 46},  {10, 47}, {11, 48}, {12, 49},
            {13, 50}, {14, 51}, {15, 52}, {16, 53}, {17, 54}, {18, 55},
    ...
    std::vector<llama_grammar_candidate> rejects =
        llama_grammar_reject_candidates_for_stack(
            grammar->rules, grammar->stacks[0], next_candidates);

    std::vector<std::vector<llama_grammar_candidate>> all_rejects;

    for (std::size_t count = 0; count < grammar->stacks.size(); ++count) {
        rejects = llama_grammar_reject_candidates_for_stack(
            grammar->rules, grammar->stacks[count], next_candidates);
    ...
    for (auto rej : all_rejects) {
        for (uint32_t i = 0; i < rej.size(); i++) {
            ...
            assert(element.index == expected_element.first &&
                   *element.code_points == expected_element.second);
        }
        index++;
    }
