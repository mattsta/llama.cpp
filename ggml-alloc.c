...
void ggml_allocr_reset(struct ggml_allocr *alloc) {
    ...
    alloc->free_blocks[0].size =
        ggml_backend_buffer_get_size(alloc->buffer) - align_offset;
}

struct ggml_allocr *ggml_allocr_new(void *data, size_t size, size_t alignment) {
    struct ggml_backend_buffer *buffer =
        ggml_backend_cpu_buffer_from_ptr(NULL, data, size);

    struct ggml_allocr *alloc =
        (struct ggml_allocr *)malloc(sizeof(struct ggml_allocr));
...
struct ggml_allocr *ggml_allocr_new_measure(size_t alignment) {
    struct ggml_allocr *alloc =
        ggml_allocr_new((void *)0x1000, (size_t)-0x1001, alignment);
    alloc->measure = true;
...
struct ggml_allocr *
ggml_allocr_new_from_buffer(struct ggml_backend_buffer *buffer) {
    struct ggml_allocr *alloc =
        (struct ggml_allocr *)malloc(sizeof(struct ggml_allocr));
...
static void init_view(struct ggml_allocr *alloc, struct ggml_tensor *view) {
    ...
    // FIXME: the view should be initialized by the owning buffer, but currently
    // this breaks the CUDA backend due to the ggml_tensor_extra_gpu ring buffer
    // overwriting the KV cache extras
    assert(ggml_allocr_is_measure(alloc) || !view->buffer ||
           view->buffer->backend == alloc->buffer->backend);
...
size_t ggml_allocr_alloc_graph_n(struct ggml_allocr *alloc,
        ...
        for (int i = 0; i < gf->n_nodes; i++) {
            ...
            if (ggml_is_view(node)) {
                ...
                if (node->buffer == NULL && node->data != NULL) {
                    // view of a pre-allocated tensor, didn't call init_view()
                    // yet
                    init_view(alloc, node);
                }
            }
            ...
            for (int j = 0; j < GGML_MAX_SRC; j++) {
                ...
                if (ggml_is_view(parent) && parent->buffer == NULL &&
                    parent->data != NULL) {
