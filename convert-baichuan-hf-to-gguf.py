...
if (
    ...
    and hparams["architectures"][0] != "BaiChuanForCausalLM"
):
...
for part_name in part_names:
    ...
    for i in range(block_count):
        if f"model.layers.{i}.self_attn.W_pack.weight" in model_part:
            ...
            tmp[f"model.layers.{i}.self_attn.q_proj.weight"] = reverse_hf_permute_part(
                model_part[f"model.layers.{i}.self_attn.W_pack.weight"],
                0,
                head_count,
                head_count,
            )
            tmp[f"model.layers.{i}.self_attn.k_proj.weight"] = reverse_hf_permute_part(
                model_part[f"model.layers.{i}.self_attn.W_pack.weight"],
                1,
                head_count,
                head_count_kv,
            )
            tmp[f"model.layers.{i}.self_attn.v_proj.weight"] = reverse_hf_part(
                model_part[f"model.layers.{i}.self_attn.W_pack.weight"], 2
            )
