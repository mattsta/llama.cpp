...
static_assert(sizeof(block_q8_0) == 2 * sizeof(float) + QK8_0,
...
static float simpleDot(const block_q4_0 &x, const block_q8_0 &y) {
    ...
    for (int i = 0; i < QK4_1 / 2; i += 2) {
        ...
        int v4 = x.qs[i + 1] >> 4;
        int j = 2 * i;
        s1 += v1 * y.qs[j] + v2 * y.qs[j + 1] + v3 * y.qs[j + 2] +
              v4 * y.qs[j + 3];
...
int main(int argc, char **argv) {
    ...
    for (int iloop = 0; iloop < nloop; ++iloop) {
        if (type == 0) {
            fillQ4blocks(x40, rndm);
        } else {
            fillQ4blocks(x41, rndm);
        }
        ...
        if (type == 0) {
            for (int i = 0; i < kVecSize; ++i) {
                s += simpleDot(x40[i], y[i]);
            }
        } else {
            for (int i = 0; i < kVecSize; ++i) {
                s += simpleDot(x41[i], y[i]);
            }
        }
        ...
        auto t =
            1e-3 * std::chrono::duration_cast<std::chrono::nanoseconds>(t2 - t1)
                       .count();
        if (iloop > 3) {
            simple.addResult(s, t);
        }

        t1 = std::chrono::high_resolution_clock::now();
        float fs;
        if (type == 0) {
            funcs.vec_dot(kVecSize * QK4_1, &fs, x40.data(), y.data());
        } else {
            funcs.vec_dot(kVecSize * QK4_1, &fs, x41.data(), y.data());
        }
