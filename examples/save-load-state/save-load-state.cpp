...
int main(int argc, char **argv) {
    ...
    // evaluate prompt
    llama_decode(ctx,
                 llama_batch_get_one(tokens.data(), tokens.size(), n_past, 0));
    n_past += tokens.size();
    ...
    // save state (rng, logits, embedding and kv_cache) to file
    {
...

        {
            ...
            llama_copy_state_data(
                ctx,
                state_mem
                    .data()); // could also copy directly to memory mapped file
    ...
    // make new context
    auto *ctx2 = llama_new_context_with_model(
        model, llama_context_params_from_gpt_params(params));
    ...
    for (auto i = 0; i < params.n_predict; i++) {
        ...
        if (llama_decode(ctx2,
                         llama_batch_get_one(&next_token, 1, n_past, 0))) {
            fprintf(stderr, "\n%s : failed to evaluate\n", __func__);
            llama_free(ctx2);
            llama_free_model(model);
            return 1;
