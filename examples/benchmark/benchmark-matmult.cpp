...
static void print_usage(int /*argc*/, char **argv,
    ...
    fprintf(stderr,
            ...
            params.n_threads);
    fprintf(stderr,
            "  -i N, --iter N     number of iterations to use during "
            "computation (default: %d)\n",
            params.n_iterations);
    fprintf(stderr, "\n");
}
...
#undef VERBOSE_DEBUGGING
    ...
    struct ggml_tensor *m2 =
        ggml_new_tensor_2d(ctx, GGML_TYPE_F32, sizex, sizez);
    ...
    ggml_quantize_chunk(qtype, (const float *)m11->data, q11->data, 0,
    ...
    // lines printf("Creating new tensor q12 & Running quantize\n");
    struct ggml_tensor *q12 = ggml_new_tensor_2d(ctx, qtype, sizex, sizey);
    ggml_quantize_chunk(qtype, (const float *)m12->data, q12->data, 0,
                        nelements, hist_cur.data());
    ...
    // Let's use the F32 result from above as a reference for the quantized
    // multiplication
    ...
    for (int i = 0; i < benchmark_params.n_iterations; i++) {
        ...
        ggml_graph_compute_helper(work_buffer, &gf31,
                                  benchmark_params.n_threads);
        ...
        printf("%9i;%8i;%6i;%6i;%6i;%15lli;%18lli;%10.2f\n", i,
               benchmark_params.n_threads, sizex, sizey, sizez,
               flops_per_matrix, usec, gflops);
        ...
        float allowed_delta = (sum_of_F32_reference) / 1000 /
                              1000; //  Let's accept an epsilon of 10^-6

        if (delta > allowed_delta) {
            printf("\nABORT - ERROR in Matrix Multiplication result - expected "
                   "%6.2f, got %6.2f (delta %6.2f > allowed_delta %6.2f)\n",
                   sum_of_F32_reference, sum_of_Q4_result, delta,
                   allowed_delta);
            exit(0);
        }

        // Running a different graph computation to make sure we override the
        // CPU cache lines
        ggml_graph_compute_helper(work_buffer, &gf32,
                                  benchmark_params.n_threads);
    }
    printf("\n");
