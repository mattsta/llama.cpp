...
static void show_additional_info(int /*argc*/, char **argv) {
    printf(
        "\n example usage: %s -m <llava-v1.5-7b/ggml-model-q5_k.gguf> --mmproj "
        "<llava-v1.5-7b/mmproj-model-f16.gguf> --image <path/to/an/image.jpg> "
        "[--temp 0.1] [-p \"describe the image in detail.\"]\n",
        argv[0]);
    printf("  note: a lower temperature value like 0.1 is recommended for "
           "better quality.\n");
}
...
int main(int argc, char **argv) {
    ...
    if (!clip_image_preprocess(ctx_clip, &img, &img_res,
                               /*pad2square =*/true)) {
    ...
    // make sure that the correct mmproj was used, i.e., compare apples to
    // apples
    const int n_llama_embd = llama_n_embd(llama_get_model(ctx_llama));

    if (n_img_embd != n_llama_embd) {
        printf("%s: embedding dim of the multimodal projector (%d) is not "
    ...
    // process the prompt
    // llava chat format is "<system_prompt>USER:
    // <image_embeddings>\n<textual_prompt>\nASSISTANT:"
    ...
    {
        const float t_img_enc_ms =
            (t_img_enc_end_us - t_img_enc_start_us) / 1000.0;

        printf("\n%s: image encoded in %8.2f ms by CLIP (%8.2f ms per image "
               "patch)\n",
