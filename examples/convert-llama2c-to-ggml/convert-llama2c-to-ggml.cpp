...
//////////////////////////////////////// llama2.c model structs and functions to
///load models, alloc memory etc.
typedef struct {
...
static void malloc_weights(TransformerWeights *w, Config *p,
                           bool shared_weights) {
    // we calloc instead of malloc to keep valgrind happy
    w->token_embedding_table = new float[p->vocab_size * p->dim]();
    printf("[%s:AK] Allocating [%d] x [%d] = [%d] float space for "
    ...
    printf(
        "[%s:AK] Allocating [%d] x [%d] x [%d] = [%d] float space for w->wq\n",
        __func__, p->n_layers, p->dim, p->dim, p->n_layers * p->dim * p->dim);

    w->wk = new float[p->n_layers * p->dim * p->dim]();
    printf(
        "[%s:AK] Allocating [%d] x [%d] x [%d] = [%d] float space for w->wk\n",
        __func__, p->n_layers, p->dim, p->dim, p->n_layers * p->dim * p->dim);

    w->wv = new float[p->n_layers * p->dim * p->dim]();
    printf(
        "[%s:AK] Allocating [%d] x [%d] x [%d] = [%d] float space for w->wv\n",
        __func__, p->n_layers, p->dim, p->dim, p->n_layers * p->dim * p->dim);

    w->wo = new float[p->n_layers * p->dim * p->dim]();
    printf(
        "[%s:AK] Allocating [%d] x [%d] x [%d] = [%d] float space for w->wo\n",
        __func__, p->n_layers, p->dim, p->dim, p->n_layers * p->dim * p->dim);

    w->w1 = new float[p->n_layers * p->hidden_dim * p->dim]();
    printf(
        "[%s:AK] Allocating [%d] x [%d] x [%d] = [%d] float space for w->w1\n",
        __func__, p->n_layers, p->hidden_dim, p->dim,
        p->n_layers * p->hidden_dim * p->dim);

    w->w2 = new float[p->n_layers * p->hidden_dim * p->dim]();
    printf(
        "[%s:AK] Allocating [%d] x [%d] x [%d] = [%d] float space for w->w2\n",
        __func__, p->n_layers, p->dim, p->hidden_dim,
        p->n_layers * p->hidden_dim * p->dim);

    w->w3 = new float[p->n_layers * p->hidden_dim * p->dim]();
    printf(
        "[%s:AK] Allocating [%d] x [%d] x [%d] = [%d] float space for w->w3\n",
        __func__, p->n_layers, p->hidden_dim, p->dim,
        p->n_layers * p->hidden_dim * p->dim);
    ...
    if (shared_weights) {
        ...
        printf(
            "[%s:AK] Allocating [%d] x [%d] = [%d] float space for w->wcls\n",
            __func__, p->vocab_size, p->dim, p->vocab_size * p->dim);
    }
}

static int checkpoint_init_weights(TransformerWeights *w, Config *p, FILE *f,
                                   bool shared_weights) {
    if (fread(w->token_embedding_table, sizeof(float), p->vocab_size * p->dim,
              f) != static_cast<size_t>(p->vocab_size * p->dim)) {
        return 1;
    }
    if (fread(w->rms_att_weight, sizeof(float), p->n_layers * p->dim, f) !=
        static_cast<size_t>(p->n_layers * p->dim)) {
        return 1;
    }
    if (fread(w->wq, sizeof(float), p->n_layers * p->dim * p->dim, f) !=
        static_cast<size_t>(p->n_layers * p->dim * p->dim)) {
        return 1;
    }
    if (fread(w->wk, sizeof(float), p->n_layers * p->dim * p->dim, f) !=
        static_cast<size_t>(p->n_layers * p->dim * p->dim)) {
        return 1;
    }
    if (fread(w->wv, sizeof(float), p->n_layers * p->dim * p->dim, f) !=
        static_cast<size_t>(p->n_layers * p->dim * p->dim)) {
        return 1;
    }
    if (fread(w->wo, sizeof(float), p->n_layers * p->dim * p->dim, f) !=
        static_cast<size_t>(p->n_layers * p->dim * p->dim)) {
        return 1;
    }
    if (fread(w->rms_ffn_weight, sizeof(float), p->n_layers * p->dim, f) !=
        static_cast<size_t>(p->n_layers * p->dim)) {
        return 1;
    }
    if (fread(w->w1, sizeof(float), p->n_layers * p->dim * p->hidden_dim, f) !=
        static_cast<size_t>(p->n_layers * p->dim * p->hidden_dim)) {
        return 1;
    }
    if (fread(w->w2, sizeof(float), p->n_layers * p->hidden_dim * p->dim, f) !=
        static_cast<size_t>(p->n_layers * p->hidden_dim * p->dim)) {
        return 1;
    }
    if (fread(w->w3, sizeof(float), p->n_layers * p->dim * p->hidden_dim, f) !=
        static_cast<size_t>(p->n_layers * p->dim * p->hidden_dim)) {
        return 1;
    }
    if (fread(w->rms_final_weight, sizeof(float), p->dim, f) !=
    ...
    if (!shared_weights &&
        fread(w->wcls, sizeof(float), p->vocab_size * p->dim, f) !=
            static_cast<size_t>(p->vocab_size * p->dim)) {
        return 1;
    }
    ...
    auto end = ftell(f);
    if (curr != end) {
        printf("Error: failed to read the checkpoint file to the end (curr = "
               "%ld, end =  %ld)\n",
               curr, end);
        return 1;
    }
...
static void print_sample_weights(TransformerWeights *w) {
    printf("----- Quick print of first of the weight vales of all the "
           "variables\n");
...
//////////////////////////////////////// ggml structs and functions required to
///load models, configs and save the model.
...
static void init_model(struct my_llama_model *model) {
    ...
    printf("[%s:GG] Allocating [%d] x [%d] = [%d] float space for "
           "model->tok_embeddings\n",
    ...
    printf(
        "[%s:GG] Allocating [%d] x[%d] = [%d] float space for model->output\n",
        __func__, n_embd, n_vocab, n_embd * n_vocab);

    // printing the per-layer allocations here so we dont print in the for loop.
    printf("[%s:GG] Allocating [%d] x[%d] = [%d] float space for layer.wq for "
           "[%d] layers\n",
           __func__, n_embd, n_embd, n_embd * n_embd, n_layer);
    printf("[%s:GG] Allocating [%d] x[%d] = [%d] float space for layer.wk for "
           "[%d] layers\n",
           __func__, n_embd, n_embd, n_embd * n_embd, n_layer);
    printf("[%s:GG] Allocating [%d] x[%d] = [%d] float space for layer.wv for "
           "[%d] layers\n",
           __func__, n_embd, n_embd, n_embd * n_embd, n_layer);
    printf("[%s:GG] Allocating [%d] x[%d] = [%d] float space for layer.wo for "
           "[%d] layers\n",
           __func__, n_embd, n_embd, n_embd * n_embd, n_layer);

    printf("[%s:GG] Allocating [%d] float space for layer.ffn_norm for [%d] "
           "layers\n",
           __func__, n_embd, n_layer);

    printf("[%s:GG] Allocating [%d] x[%d] = [%d] float space for layer.w1 for "
           "[%d] layers\n",
           __func__, n_ff, n_embd, n_embd * n_ff, n_layer);
    printf("[%s:GG] Allocating [%d] x[%d] = [%d] float space for layer.w2 for "
           "[%d] layers\n",
           __func__, n_embd, n_ff, n_ff * n_embd, n_layer);
    printf("[%s:GG] Allocating [%d] x[%d] = [%d] float space for layer.w3 for "
           "[%d] layers\n",
           __func__, n_ff, n_embd, n_embd * n_ff, n_layer);
    ...
    for (uint32_t i = 0; i < n_layer; ++i) {
        ...
        ggml_set_name(layer.attention_norm,
                      (layers_i + ".attention_norm.weight").c_str());
        ...
        ggml_format_name(layer.w1, "%s.feed_forward.w1.weight",
                         layers_i.c_str());
        ggml_format_name(layer.w2, "%s.feed_forward.w2.weight",
                         layers_i.c_str());
        ggml_format_name(layer.w3, "%s.feed_forward.w3.weight",
                         layers_i.c_str());
    }
}

static float get_f32_2d(struct ggml_tensor *tensor, int64_t i0, int64_t i1) {
    float *ptr = (float *)((char *)tensor->data + i0 * tensor->nb[0] +
                           i1 * tensor->nb[1]);
    return *ptr;
}

static int32_t get_i32_2d(struct ggml_tensor *tensor, int64_t i0, int64_t i1) {
    int32_t *ptr = (int32_t *)((char *)tensor->data + i0 * tensor->nb[0] +
                               i1 * tensor->nb[1]);
    return *ptr;
}
...
static void load_vocab(const char *filename, Config *config,
...
static void convert_weights_ak_to_gg(struct ggml_tensor *gg_weights,
                                     const float *karpathy_weights) {
    int ct;
    ...
    case 1:
        ...
        for (int i0 = 0; i0 < gg_weights->ne[0]; i0++) {
            float *ptr =
                (float *)((char *)gg_weights->data + i0 * gg_weights->nb[0]);
    ...
    case 2:
        ...
        for (int i1 = 0; i1 < gg_weights->ne[1]; i1++) {
            for (int i0 = 0; i0 < gg_weights->ne[0]; i0++) {
                float *ptr =
                    (float *)((char *)gg_weights->data +
                              i0 * gg_weights->nb[0] + i1 * gg_weights->nb[1]);
                *ptr = karpathy_weights[ct];
                ct++;
            }
        }
    ...
    case 3:
        ...
        for (int i2 = 0; i2 < gg_weights->ne[2]; i2++) {
            for (int i1 = 0; i1 < gg_weights->ne[1]; i1++) {
                for (int i0 = 0; i0 < gg_weights->ne[0]; i0++) {
                    float *ptr = (float *)((char *)gg_weights->data +
                                           i0 * gg_weights->nb[0] +
                                           i1 * gg_weights->nb[1] +
                                           i2 * gg_weights->nb[2]);
                    *ptr = karpathy_weights[ct];
...
static void save_as_llama_model(struct llama_vocab *vocab,
                                struct my_llama_model *model,
                                TransformerWeights *w, const char *filename) {
    // convert AK weights into GG weights one by one.
    // w->token_embedding_table -> model->tok_embeddings
    // float*                   -> struct ggml_tensor
    convert_weights_ak_to_gg(model->tok_embeddings, w->token_embedding_table);
    convert_weights_ak_to_gg(model->output,
                             w->wcls ? w->wcls : w->token_embedding_table);
    ...
    for (uint32_t i = 0; i < model->hparams.n_layer; ++i) {
        auto &layer = model->layers[i];
        // 1d
        convert_weights_ak_to_gg(layer.attention_norm,
                                 &w->rms_att_weight[i * row_length]);
        convert_weights_ak_to_gg(layer.ffn_norm,
                                 &w->rms_ffn_weight[i * row_length]);
    ...
    gguf_set_arr_data(ctx, KV_TOKENIZER_SCORES, GGUF_TYPE_FLOAT32,
                      scores.data(), scores.size());
    gguf_set_arr_data(ctx, KV_TOKENIZER_TOKEN_TYPE, GGUF_TYPE_INT32,
...
static void print_usage(int /*argc*/, char **argv,
    ...
    fprintf(stderr,
            "  --copy-vocab-from-model FNAME    path of gguf llama model or "
            "llama2.c vocabulary from which to copy vocab (default '%s')\n",
            params->fn_vocab_model);
    fprintf(stderr, "  --llama2c-model FNAME            [REQUIRED] model path "
                    "from which to load Karpathy's llama2.c model\n");
    fprintf(stderr,
            "  --llama2c-output-model FNAME     model path to save the "
            "converted llama2.c model (default %s')\n",
...
static bool params_parse(int argc, char **argv, struct train_params *params) {
    ...
    if (!reqd_param_found) {
        fprintf(stderr, "error: please specify a llama2.c .bin file to be "
                        "converted with argument --llama2c-model\n");
        print_usage(argc, argv, &default_params);
        exit(1);
    }
...
int main(int argc, char **argv) {
    ...
    model.hparams.n_rot = std::min((uint32_t)params.n_rotmax,
                                   model.hparams.n_embd / model.hparams.n_head);
    print_params(&model.hparams);
    struct ggml_init_params lcparams;
    lcparams.mem_size =
        1024ll * 1024ll * 1024ll * ((size_t)params.mem_model_gb);
    ...
    save_as_llama_model(&vocab, &model, &weights,
                        params.fn_llama2c_output_model);
