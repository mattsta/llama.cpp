...
static const char *LLM_KV_TOKENIZER_SEP_ID =
...
static void alloc_model(struct ggml_allocr *alloc,
...
static void init_model(struct my_llama_model *model) {
    ...
    model->tok_embeddings =
        ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n_embd, n_vocab);
    ...
    alloc = ggml_allocr_new(model->data.data(), model->data.size(),
                            tensor_alignment);
    alloc_model(alloc, model);
    ggml_allocr_free(alloc);
}

static void randomize_model(struct my_llama_model *model, int seed, float mean,
...
static struct ggml_tensor *llama_build_train_graphs(
    struct my_llama_model *model, struct ggml_allocr *alloc,
    ...
    struct ggml_tensor *t01 = ggml_get_rows(ctx, model->tok_embeddings, t00);
    set_name(t01, "t01");
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        struct ggml_tensor *t02 = ggml_rms_norm(ctx, cur, f_norm_rms_eps);
        set_name(t02, "t02");
        assert_shape_2d(t02, n_embd, N * n_batch);
        struct ggml_tensor *t03 = ggml_repeat(ctx, layer.attention_norm, t02);
        set_name(t03, "t03");
        ...
        struct ggml_tensor *t06 =
            ggml_reshape_4d(ctx, t05, n_embd / n_head, n_head, N, n_batch);
        set_name(t06, "t06");
        ...
        struct ggml_tensor *t09 =
            ggml_reshape_4d(ctx, t08, n_embd / n_head, n_head, N, n_batch);
        set_name(t09, "t09");
        ...
        struct ggml_tensor *t12 =
            ggml_reshape_4d(ctx, t11, N, n_batch, n_embd / n_head, n_head);
        set_name(t12, "t12");
    ...
    if (alloc) {
        ...
        for (int i = 0; i < (int)checkpoints.size(); ++i) {
            if (checkpoints[i]->data == NULL &&
                checkpoints[i]->view_src == NULL) {
...
static void load_llama_model_gguf(struct gguf_context *fctx,
    ...
    // n_ctx was not saved in earlier checkpoint file versions, so we make it
    // optional here
    GGUF_GET_KEY(fctx, model->hparams.n_ctx, gguf_get_val_u32, GGUF_TYPE_UINT32,
                 false, kv(LLM_KV_CONTEXT_LENGTH));

    GGUF_GET_KEY(fctx, model->hparams.n_embd, gguf_get_val_u32,
                 GGUF_TYPE_UINT32, true, kv(LLM_KV_EMBEDDING_LENGTH));
    GGUF_GET_KEY(fctx, model->hparams.n_ff, gguf_get_val_u32, GGUF_TYPE_UINT32,
                 true, kv(LLM_KV_FEED_FORWARD_LENGTH));
    GGUF_GET_KEY(fctx, model->hparams.n_head, gguf_get_val_u32,
                 GGUF_TYPE_UINT32, true, kv(LLM_KV_ATTENTION_HEAD_COUNT));
    GGUF_GET_KEY(fctx, model->hparams.n_layer, gguf_get_val_u32,
                 GGUF_TYPE_UINT32, true, kv(LLM_KV_BLOCK_COUNT));

    model->hparams.n_rot = model->hparams.n_embd / model->hparams.n_head;
    GGUF_GET_KEY(fctx, model->hparams.n_rot, gguf_get_val_u32, GGUF_TYPE_UINT32,
                 false, kv(LLM_KV_ROPE_DIMENSION_COUNT));

    float rope_freq_scale = 1.0f;
    GGUF_GET_KEY(fctx, model->hparams.f_norm_rms_eps, gguf_get_val_f32,
                 GGUF_TYPE_FLOAT32, false,
                 kv(LLM_KV_ATTENTION_LAYERNORM_RMS_EPS));
    GGUF_GET_KEY(fctx, model->hparams.rope_freq_base, gguf_get_val_f32,
                 GGUF_TYPE_FLOAT32, false, kv(LLM_KV_ROPE_FREQ_BASE));
    GGUF_GET_KEY(fctx, rope_freq_scale, gguf_get_val_f32, GGUF_TYPE_FLOAT32,
    ...
    copy_tensor_by_name(model->tok_embeddings, f_ggml_ctx,
    ...
    for (uint32_t i = 0; i < model->hparams.n_layer; ++i) {
        ...
        copy_tensor_by_name(layer.attention_norm, f_ggml_ctx,
                            tni(LLM_TENSOR_ATTN_NORM, i));
        ...
        copy_tensor_by_name(layer.ffn_norm, f_ggml_ctx,
                            tni(LLM_TENSOR_FFN_NORM, i));
...
static void save_llama_model_gguf(struct gguf_context *fctx,
    ...
    gguf_set_val_f32(fctx, kv(LLM_KV_ROPE_FREQ_BASE),
                     model->hparams.rope_freq_base); // TODO load in llama.cpp
    gguf_set_val_f32(fctx, kv(LLM_KV_ROPE_SCALE_LINEAR),
                     1.0f / model->hparams.rope_freq_scale);
    ...
    // set vocab by copying from vocab_model gguf file
    {
        ...
        const int toktype_idx =
            gguf_find_key(vctx, kv(LLM_KV_TOKENIZER_TOKEN_TYPE));
        if (toktype_idx == -1) {
        ...
        GGUF_GET_KEY(vctx, tokenizer_name, gguf_get_val_str, GGUF_TYPE_STRING,
                     true, kv(LLM_KV_TOKENIZER_MODEL));

        gguf_set_val_str(fctx, kv(LLM_KV_TOKENIZER_MODEL),
                         tokenizer_name.c_str());
        gguf_set_arr_data(fctx, kv(LLM_KV_TOKENIZER_SCORES), GGUF_TYPE_FLOAT32,
                          scores, n_vocab);
        gguf_set_arr_data(fctx, kv(LLM_KV_TOKENIZER_TOKEN_TYPE),
                          GGUF_TYPE_INT32, toktypes, n_vocab);
        ...
        if (tokenizer_name == "llama") {
            ...
            const int merges_keyidx =
                gguf_find_key(vctx, kv(LLM_KV_TOKENIZER_MERGES));
            if (merges_keyidx == -1) {
            ...
            gguf_set_arr_str(fctx, kv(LLM_KV_TOKENIZER_MERGES), merges.data(),
                             n_merges);
            ...
            fprintf(stderr, "%s: unknown tokenizer: '%s'", __func__,
                    tokenizer_name.c_str());
        ...
        gguf_set_arr_str(fctx, kv(LLM_KV_TOKENIZER_LIST), tokens.data(),
                         n_vocab);

        GGUF_GET_KEY(vctx, special_bos_id, gguf_get_val_u32, GGUF_TYPE_UINT32,
                     false, kv(LLM_KV_TOKENIZER_BOS_ID));
        GGUF_GET_KEY(vctx, special_eos_id, gguf_get_val_u32, GGUF_TYPE_UINT32,
                     false, kv(LLM_KV_TOKENIZER_EOS_ID));
        GGUF_GET_KEY(vctx, special_unk_id, gguf_get_val_u32, GGUF_TYPE_UINT32,
                     false, kv(LLM_KV_TOKENIZER_UNK_ID));
        GGUF_GET_KEY(vctx, special_sep_id, gguf_get_val_u32, GGUF_TYPE_UINT32,
                     false, kv(LLM_KV_TOKENIZER_SEP_ID));
        GGUF_GET_KEY(vctx, special_pad_id, gguf_get_val_u32, GGUF_TYPE_UINT32,
...
static void save_llama_model_file(const char *filename,
                                  const char *fn_vocab_model,
                                  struct my_llama_model *model) {
...
static void load_checkpoint_gguf(struct gguf_context *fctx,
                                 struct ggml_context *f_ggml_ctx,
                                 struct my_llama_model *model,
    ...
    if (load_train_state_gguf(fctx, f_ggml_ctx, train)) {
        ...
        GGUF_GET_KEY(fctx, train_type, gguf_get_val_str, GGUF_TYPE_STRING,
                     false, LLM_KV_TRAINING_TYPE);
...
static void save_checkpoint_gguf(struct gguf_context *fctx,
                                 const char *fn_vocab_model,
                                 struct my_llama_model *model,
                                 struct train_state *train) {
    gguf_set_val_str(fctx, LLM_KV_TRAINING_TYPE,
                     LLM_KV_TRAINING_TYPE_TRAIN_MODEL);
    save_llama_model_gguf(fctx, fn_vocab_model, model);
    save_train_state_gguf(fctx, train);
}

static bool load_checkpoint_file(const char *filename,
                                 struct my_llama_model *model,
                                 struct train_state *train) {
    struct ggml_context *f_ggml_ctx;
    struct gguf_init_params params;
...
static void save_checkpoint_file(const char *filename,
                                 const char *fn_vocab_model,
                                 struct my_llama_model *model,
                                 struct train_state *train) {
    printf("%s: saving to %s\n", __func__, filename);
...
static void train_print_usage(int argc, char **argv,
    ...
    fprintf(stderr,
            "  --vocab-model FNAME        model path from which to load vocab "
            "(default '%s')\n",
            params->fn_vocab_model);
    fprintf(
        stderr,
        "  --model-out FNAME          path to save ggml model (default '%s')\n",
        params->fn_model_out);
    fprintf(stderr, "  --only-write-model         only save llama model, don't "
                    "do any training. use this if you only want to convert a "
                    "checkpoint to a model.\n");
    fprintf(stderr,
            "  --embd N                   Embedding size used for new models "
            "(default %d)\n",
            params->n_embd);
    fprintf(stderr,
            "  --ff N                     Feedforward size used for new "
            "models. (default %d)\n",
            params->n_ff);
    fprintf(stderr,
            "  --head N                   Number of heads for new models "
            "(default %d)\n",
            params->n_head);
    fprintf(stderr,
            "  --layer N                  Number of layers for new models "
            "(default %d)\n",
            params->n_layer);
    fprintf(
        stderr,
...
static void save_train_files(void *vdata, struct train_state *train) {
    ...
    if (strlen(data->fn_checkpoint_out) > 0) {
        save_checkpoint_file(get_train_filename(data->fn_checkpoint_out,
                                                data->pattern_fn_it,
                                                data->fn_latest, iter)
                                 .c_str(),
                             data->fn_vocab_model, data->model, train);
        save_checkpoint_file(get_train_filename(data->fn_checkpoint_out,
                                                data->pattern_fn_it,
                                                data->fn_latest, -1)
                                 .c_str(),
    ...
    if (strlen(data->fn_model_out) > 0) {
        save_llama_model_file(get_train_filename(data->fn_model_out,
                                                 data->pattern_fn_it,
                                                 data->fn_latest, iter)
                                  .c_str(),
                              data->fn_vocab_model, data->model);
        save_llama_model_file(get_train_filename(data->fn_model_out,
                                                 data->pattern_fn_it,
                                                 data->fn_latest, -1)
                                  .c_str(),
...
int main(int argc, char **argv) {
    ...
    struct llama_model *lmodel =
        llama_load_model_from_file(params.fn_vocab_model, mparams);
    ...
    printf("%s: init model\n", __func__);
    bool existed =
        load_checkpoint_file(params.common.fn_checkpoint_in, &model, train);
    if (existed) {
        ...
        if (opt_past_changed) {
            die("Optimizer parameter '--opt-past N' differs from checkpoint "
                "file. To use different value train from scratch with empty "
    ...
    printf("%s: model_size = %zu bytes (%.1f MB)\n", __func__,
           (ggml_used_mem(model.ctx) + model.data.size()),
           (float)(ggml_used_mem(model.ctx) + model.data.size()) /
               (1024.0f * 1024.0f));
    ...
    for (unsigned order = 0; order < (unsigned)GGML_CGRAPH_EVAL_ORDER_COUNT;
        ...
        loss = llama_build_train_graphs(
            &model, alloc, ctx_compute, gf, gb, gb_tmp, &logits, tokens_input,
    ...
    loss = llama_build_train_graphs(&model, alloc, ctx_compute, gf, gb, gb_tmp,
                                    &logits, tokens_input, target_probs,
                                    n_tokens, n_batch, params.common.use_flash,
                                    params.common.use_checkpointing);
    ggml_allocr_free(alloc);
