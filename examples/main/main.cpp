...
int main(int argc, char **argv) {
    ...
    if (!session_tokens.empty()) {
        ...
        if (params.prompt.empty() &&
            ...
            LOG_TEE("%s: warning: session file has low similarity to prompt "
            ...
            if (llama_sampling_last(ctx_sampling) == llama_token_eos(model)) {
                ...
                if (params.interactive) {
                    if (!params.antiprompt.empty()) {
                        ...
                        const auto first_antiprompt = ::llama_tokenize(
                            ctx, params.antiprompt.front(), false, true);
                        embd_inp.insert(embd_inp.end(),
            ...
            if (n_past > 0 && is_interacting) {
                ...
                if (buffer.length() > 1) {
                    ...
                    if (params.instruct && !is_antiprompt) {
                        ...
                        embd_inp.insert(embd_inp.end(), inp_pfx.begin(),
                                        inp_pfx.end());
                    }
                    if (params.escape) {
                    ...
                    const auto line_inp =
                        ::llama_tokenize(ctx, buffer, false, false);
                    const auto line_sfx =
                        ::llama_tokenize(ctx, params.input_suffix, false, true);
                    LOG("input tokens: %s\n",
                        LOG_TOKENS_TOSTR_PRETTY(ctx, line_inp).c_str());

                    embd_inp.insert(embd_inp.end(), line_pfx.begin(),
                                    line_pfx.end());
                    embd_inp.insert(embd_inp.end(), line_inp.begin(),
                                    line_inp.end());
                    embd_inp.insert(embd_inp.end(), line_sfx.begin(),
                                    line_sfx.end());

                    // instruct mode: insert response suffix
                    if (params.instruct) {
                        LOG("inserting instruction suffix\n");
                        embd_inp.insert(embd_inp.end(), inp_sfx.begin(),
                                        inp_sfx.end());
                    }
