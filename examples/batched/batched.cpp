...
int main(int argc, char **argv) {
    ...
    const int n_kv_req =
        tokens_list.size() + (n_len - tokens_list.size()) * n_parallel;
    ...
    LOG_TEE("\n%s: n_len = %d, n_ctx = %d, n_batch = %d, n_parallel = %d, "
            "n_kv_req = %d\n",
    ...
    // tokens
    if (n_kv_req > n_ctx) {
        LOG_TEE("%s: error: n_kv_req (%d) > n_ctx, the required KV cache size "
                "is not big enough\n",
                __func__, n_kv_req);
    ...
    llama_batch batch = llama_batch_init(
        std::max(tokens_list.size(), (size_t)n_parallel), 0, 1);
    ...
    // this way, the parallel sequences will "reuse" the prompt tokens without
    // having to copy them
    ...
    while (n_cur <= n_len) {
        ...
        for (int32_t i = 0; i < n_parallel; ++i) {
            ...
            if (new_token_id == llama_token_eos(model) || n_cur == n_len) {
                ...
                LOG_TEE("\n");
                if (n_parallel > 1) {
                    LOG_TEE("%s: stream %d finished at n_cur = %d", __func__, i,
                            n_cur);
                }
