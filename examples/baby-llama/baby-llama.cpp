...
static struct ggml_tensor *randomize_tensor(struct ggml_tensor *tensor,
                                            int ndims, const int64_t ne[],
    ...
    case 3:
        for (int i2 = 0; i2 < ne[2]; i2++) {
            for (int i1 = 0; i1 < ne[1]; i1++) {
                for (int i0 = 0; i0 < ne[0]; i0++) {
                    ((float *)
                         tensor->data)[i2 * ne[1] * ne[0] + i1 * ne[0] + i0] =
                        frand() * (fmax - fmin) + fmin;
                }
            }
        }
...
static uint32_t get_n_ff(const struct llama_hparams *hparams) {
    const uint32_t n_ff =
        ((2 * (4 * hparams->n_embd) / 3 + hparams->n_mult - 1) /
         hparams->n_mult) *
        hparams->n_mult;
    return n_ff;
}
...
static void init_model(struct llama_model *model) {
    ...
    model->tok_embeddings = ggml_new_tensor_2d(
        ctx, GGML_TYPE_F32, n_embd,
        n_vocab); // ("tok_embeddings.weight", {n_embd, n_vocab});
    model->norm = ggml_new_tensor_1d(ctx, GGML_TYPE_F32,
                                     n_embd); // ("norm.weight", {n_embd});
    model->output = ggml_new_tensor_2d(
        ctx, GGML_TYPE_F32, n_embd,
        n_vocab); // ("output.weight",         {n_embd, n_vocab});
    ...
    for (uint32_t i = 0; i < n_layer; ++i) {
        ...
        layer.attention_norm = ggml_new_tensor_1d(
            ctx, GGML_TYPE_F32,
            n_embd); // (layers_i + ".attention_norm.weight", {n_embd});

        layer.wq = ggml_new_tensor_2d(
            ctx, GGML_TYPE_F32, n_embd,
            n_embd); // (layers_i + ".attention.wq.weight", {n_embd, n_embd});
        layer.wk = ggml_new_tensor_2d(
            ctx, GGML_TYPE_F32, n_embd,
            n_embd); // (layers_i + ".attention.wk.weight", {n_embd, n_embd});
        layer.wv = ggml_new_tensor_2d(
            ctx, GGML_TYPE_F32, n_embd,
            n_embd); // (layers_i + ".attention.wv.weight", {n_embd, n_embd});
        layer.wo = ggml_new_tensor_2d(
            ctx, GGML_TYPE_F32, n_embd,
            n_embd); // (layers_i + ".attention.wo.weight", {n_embd, n_embd});

        layer.ffn_norm = ggml_new_tensor_1d(
            ctx, GGML_TYPE_F32,
            n_embd); // (layers_i + ".ffn_norm.weight", {n_embd});

        layer.w1 = ggml_new_tensor_2d(
            ctx, GGML_TYPE_F32, n_embd,
            n_ff); // (layers_i + ".feed_forward.w1.weight", {n_embd,   n_ff});
        layer.w2 = ggml_new_tensor_2d(
            ctx, GGML_TYPE_F32, n_ff,
            n_embd); // (layers_i + ".feed_forward.w2.weight", {  n_ff,
                     // n_embd});
        layer.w3 = ggml_new_tensor_2d(
            ctx, GGML_TYPE_F32, n_embd,
            n_ff); // (layers_i + ".feed_forward.w3.weight", {n_embd,   n_ff});
    }
}
...
static void init_model_lora(struct llama_model_lora *model) {
    ...
    const uint32_t n_ff =
        ((2 * (4 * n_embd) / 3 + n_mult - 1) / n_mult) * n_mult;
    ...
    model->norm = ggml_new_tensor_1d(ctx, GGML_TYPE_F32,
                                     n_embd); // ("norm.weight", {n_embd});
    model->outputa = ggml_new_tensor_2d(
        ctx, GGML_TYPE_F32, n_lora,
        n_vocab); // ("output.weight",         {n_embd, n_vocab});
    model->outputb = ggml_new_tensor_2d(
        ctx, GGML_TYPE_F32, n_embd,
        n_lora); // ("output.weight",         {n_embd, n_vocab});
    ...
    for (uint32_t i = 0; i < n_layer; ++i) {
        ...
        layer.wqa = ggml_new_tensor_2d(
            ctx, GGML_TYPE_F32, n_lora,
            n_embd); // (layers_i + ".attention.wq.weight", {n_embd, n_embd});
        layer.wqb = ggml_new_tensor_2d(
            ctx, GGML_TYPE_F32, n_embd,
            n_lora); // (layers_i + ".attention.wq.weight", {n_embd, n_embd});
        layer.wka = ggml_new_tensor_2d(
            ctx, GGML_TYPE_F32, n_lora,
            n_embd); // (layers_i + ".attention.wk.weight", {n_embd, n_embd});
        layer.wkb = ggml_new_tensor_2d(
            ctx, GGML_TYPE_F32, n_embd,
            n_lora); // (layers_i + ".attention.wk.weight", {n_embd, n_embd});
        layer.wva = ggml_new_tensor_2d(
            ctx, GGML_TYPE_F32, n_lora,
            n_embd); // (layers_i + ".attention.wv.weight", {n_embd, n_embd});
        layer.wvb = ggml_new_tensor_2d(
            ctx, GGML_TYPE_F32, n_embd,
            n_lora); // (layers_i + ".attention.wv.weight", {n_embd, n_embd});
        layer.woa = ggml_new_tensor_2d(
            ctx, GGML_TYPE_F32, n_lora,
            n_embd); // (layers_i + ".attention.wo.weight", {n_embd, n_embd});
        layer.wob = ggml_new_tensor_2d(
            ctx, GGML_TYPE_F32, n_embd,
            n_lora); // (layers_i + ".attention.wo.weight", {n_embd, n_embd});
...
static void randomize_model(struct llama_model *model, int seed, float mean,
...
static void randomize_model_lora(struct llama_model_lora *model, int seed,
...
static void init_kv_cache(struct llama_kv_cache *cache,
                          struct llama_model *model, int n_batch) {
    const auto &hparams = model->hparams;
    ...
    if (!cache->ctx) {
        struct ggml_init_params params;
        params.mem_size =
            2u * n_elements * ggml_type_size(GGML_TYPE_F32) + 2u * 1024 * 1024;
...
static bool init_kv_cache_lora(struct llama_kv_cache *cache,
                               struct llama_model_lora *model, int n_batch) {
    const auto &hparams = model->hparams;
...
static struct ggml_tensor *forward(struct llama_model *model,
    ...
    struct ggml_tensor *inpL =
        ggml_get_rows(ctx0, model->tok_embeddings, tokens);
    for (int il = 0; il < n_layer; ++il) {
        ...
        // self-attention
        {
            ...
            // store key and value to memory
            {
                ...
                struct ggml_tensor *Vcur = ggml_cont(
                    ...
                    ggml_transpose(
                        ctx0,
                        ggml_reshape_2d(
                            ctx0, ggml_mul_mat(ctx0, model->layers[il].wv, cur),
                            n_embd, N)));

                // kv_self.k shape [n_embd * n_ctx * n_layer, 1]
                // kv_self.v shape [n_embd * n_ctx * n_layer, 1]
                // k         shape [n_embd * N, 1]   ==
                // kv_self.k[:,n_past:n_past+N,il,0] v         shape [N, n_embd,
                // 1, 1] == kv_self.v[:,n_past:n_past+N,il,0]

                /* {
                    struct ggml_tensor * k = ggml_view_1d(ctx0, kv_self.k,
                N*n_embd, (ggml_element_size(kv_self.k)*n_embd)*(il*n_ctx +
                n_past)); struct ggml_tensor * v = ggml_view_2d(ctx0, kv_self.v,
                N, n_embd, (   n_ctx)*ggml_element_size(kv_self.v),
                ...
                kc = ggml_set_1d(ctx0, kc,
                                 ggml_reshape_1d(ctx0, Kcur, n_embd * N),
                                 (ggml_element_size(kv_self.k) * n_embd) *
                                     (il * n_ctx + n_past));
                vc = ggml_set_2d(
                    ctx0, vc, Vcur, (n_ctx)*ggml_element_size(kv_self.v),
                    (il * n_ctx) * ggml_element_size(kv_self.v) * n_embd +
                        n_past * ggml_element_size(kv_self.v));
            }
            ...
            struct ggml_tensor *K = ggml_permute(
                ...
                ggml_reshape_3d(
                    ctx0,
                    ggml_view_1d(ctx0, kc, (n_past + N) * n_embd,
                                 il * n_ctx * ggml_element_size(kc) * n_embd),
            ...
            // V shape [n_past + N, n_embd/n_head, n_head, 1] ==
            // kv_self.v[:,:(n_past+N),il,1]
            struct ggml_tensor *V =
                ggml_view_3d(ctx0, vc, n_past + N, n_embd / n_head, n_head,
...
forward_batch(struct llama_model *model, struct llama_kv_cache *cache,
              struct ggml_context *ctx0, struct ggml_cgraph *gf,
              struct ggml_tensor *tokens_input, const int n_tokens,
              const int n_past, const int n_batch) {
    const int N = n_tokens;
    ...
    struct ggml_tensor *tokens =
        ggml_new_tensor_1d(ctx0, GGML_TYPE_I32, N * n_batch);
    memcpy(tokens->data, tokens_input->data,
           ggml_element_size(tokens) * N * n_batch);
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // self-attention
        {
            ...
            // store key and value to memory
            {
                ...
                // k         shape [n_embd * N, n_batch]   ==
                // kv_self.k[:,n_past:n_past+N,:,il] v         shape [N, n_embd,
                // n_batch, 1] == kv_self.v[:,n_past:n_past+N,:,il]
                ...
                kc = ggml_set_2d(
                    ctx0, kc, ggml_reshape_2d(ctx0, Kcur, n_embd * N, n_batch),
                    ggml_element_size(kc) * n_embd * n_ctx,
                    (ggml_element_size(kc) * n_embd) *
                        (il * n_batch * n_ctx + n_past));
                vc = ggml_set_2d(
                    ctx0, vc, ggml_reshape_2d(ctx0, Vcur, N * n_embd, n_batch),
                    ggml_element_size(vc) * n_ctx * n_embd,
                    ggml_element_size(vc) *
                        (n_past + il * n_embd * n_batch * n_ctx));
            ...
            struct ggml_tensor *K = ggml_permute(
                ...
                ggml_reshape_4d(
                    ctx0,
                    ggml_view_3d(ctx0, kc, n_embd, (n_past + N), n_batch,
                                 n_embd * ggml_element_size(kc),
                                 n_ctx * n_embd * ggml_element_size(kc),
                                 il * n_batch * n_ctx * n_embd *
                                     ggml_element_size(kc)),
            ...
            // split cached V into n_head heads
            // kv_self.v shape [n_ctx * n_embd * n_batch * n_layer]
            // V shape [n_past + N, n_embd/n_head, n_head, n_batch] ==
            // kv_self.v[:(n_past+N),:,:,il]
            struct ggml_tensor *V = ggml_view_4d(
                ctx0, vc, n_past + N, n_embd / n_head, n_head, n_batch,
            ...
            // cur = KQV_merged.contiguous().view(n_embd, N)
            // cur shape [n_embd,N*n_batch,1,1]
            cur = ggml_reshape_2d(ctx0, ggml_cont(ctx0, KQV_merged), n_embd,
                                  N * n_batch);
            assert_shape_2d(cur, n_embd, N * n_batch);
            // cur = ggml_cpy(ctx0,
            //         KQV_merged,
...

    {
        ...
        inpL = ggml_reshape_3d(ctx0, inpL, n_vocab, N, n_batch);
...
static struct ggml_tensor *forward_lora(struct llama_model_lora *model,
                                        struct llama_kv_cache *cache,
                                        struct ggml_context *ctx0,
                                        struct ggml_cgraph *gf,
                                        struct ggml_tensor *tokens_input,
        ...
        // self-attention
        {
            ...
            struct ggml_tensor *Kcur = ggml_rope(
                ...
                ggml_reshape_3d(
                    ctx0,
                    ggml_mul_mat(
                        ctx0, model->layers[il].wka,
                        ggml_mul_mat(ctx0, model->layers[il].wkb, cur)),
                    n_embd / n_head, n_head, N),
                KQ_pos, n_rot, 0, 0);
                ...
                struct ggml_tensor *Vcur = ggml_cont(
                    ...
                    ggml_transpose(
                        ...
                        ggml_reshape_2d(
                            ctx0,
                            ggml_mul_mat(
                                ctx0, model->layers[il].wva,
                                ggml_mul_mat(ctx0, model->layers[il].wvb, cur)),
                            n_embd, N)));
            ...
            // projection (no bias)
            // cur shape [n_embd,N,1,1]
            cur = ggml_mul_mat(ctx0, model->layers[il].woa,
                               ggml_mul_mat(ctx0, model->layers[il].wob, cur));
        }
    ...
    // lm_head
    // inpL shape [n_vocab,N,1,1]
    inpL = ggml_mul_mat(ctx0, model->outputa,
                        ggml_mul_mat(ctx0, model->outputb, inpL));
...
static void sample_softmax(struct ggml_tensor *logits,
...
static void sample_softmax_batch(struct ggml_context *ctx,
    ...
    for (int k = 0; k < n_batch; ++k) {
        struct ggml_tensor *best_samples_k = ggml_view_1d(
            ctx, best_samples, best_samples->ne[0], k * best_samples->nb[1]);
        struct ggml_tensor *logits_k =
            ggml_view_2d(ctx, logits, logits->ne[0], logits->ne[1],
                         logits->nb[1], k * logits->nb[2]);
        struct ggml_tensor *probs_k =
            ggml_view_2d(ctx, probs, probs->ne[0], probs->ne[1], probs->nb[1],
                         k * probs->nb[2]);
...
static void get_example_targets(int example_id,
                                struct ggml_tensor *tokens_input,
    ...
    for (int i = 1; i < n_tokens + 1; ++i) {
        ...
        int token = std::max(
            1, std::min(1 + (int)(z * (float)(n_vocab - 1)), n_vocab - 1));
...
static void get_example_targets_batch(struct ggml_context *ctx, int example_id,
                                      struct ggml_tensor *tokens_input,
                                      struct ggml_tensor *targets) {
    ...
    for (int k = 0; k < n_batch; ++k) {
        struct ggml_tensor *tokens_input_k = ggml_view_1d(
            ctx, tokens_input, tokens_input->ne[0], k * tokens_input->nb[1]);
        struct ggml_tensor *targets_k =
            ggml_view_2d(ctx, targets, targets->ne[0], targets->ne[1],
                         targets->nb[1], k * targets->nb[2]);
        get_example_targets(example_id * n_batch + k, tokens_input_k,
                            targets_k);
    }
}

static void lshift_examples(struct ggml_tensor *tokens_input,
                            struct ggml_tensor *targets, int n_shift) {
    int n_tokens = tokens_input->ne[0];
    int n_vocab = targets->ne[0];
    for (int i = 0; i < n_tokens - n_shift; ++i) {
        ggml_set_i32_1d(tokens_input, i,
                        ggml_get_i32_1d(tokens_input, i + n_shift));
        for (int k = 0; k < n_vocab; ++k) {
            ggml_set_f32_1d(
                targets, i * n_vocab + k,
                ggml_get_f32_1d(targets, (i + n_shift) * n_vocab + k));
        }
    }
}

static struct ggml_tensor *square_error_loss(struct ggml_context *ctx,
...
static struct ggml_tensor *cross_entropy_loss(struct ggml_context *ctx,
                                              struct ggml_tensor *a,
                                              struct ggml_tensor *b) {
    ...
    return ggml_sum(
        ...
        ggml_neg(
            ...
            ggml_sum_rows(
                ctx,
                ggml_mul(ctx, ggml_soft_max(ctx, a),
                         ggml_log(ctx, ggml_add1(ctx, ggml_soft_max(ctx, b),
...
int main(int argc, char **argv) {
    ...
    model.hparams.n_rot =
        std::min(16u, model.hparams.n_embd / model.hparams.n_head);
    ...
    /*
        ...
        model_lora.hparams.n_rot   = MIN(16, model_lora.hparams.n_embd /
       model_lora.hparams.n_head);
        // model.hparams.n_rot   = (model.hparams.n_embd / model.hparams.n_head)
       / 2;
    ...
    */
    ...
    for (int ex = 0; ex < n_examples; ++ex) {
        ...
        struct ggml_tensor *after_opt_best_samples =
            ggml_new_tensor_2d(ctx0, GGML_TYPE_I32, n_tokens, n_batch);
        struct ggml_tensor *after_opt_probs =
            ggml_new_tensor_3d(ctx0, GGML_TYPE_F32, n_vocab, n_tokens, n_batch);
        struct ggml_tensor *tokens_input =
            ggml_new_tensor_2d(ctx0, GGML_TYPE_I32, n_tokens, n_batch);
        struct ggml_tensor *targets =
            ggml_new_tensor_3d(ctx0, GGML_TYPE_F32, n_vocab, n_tokens, n_batch);
        ...
        struct ggml_tensor *logits =
            forward_batch(&model, &kv_self, ctx0, &gf, tokens_input, n_tokens,
                          n_past, n_batch);
        ...
        struct ggml_opt_params opt_params_lbfgs =
            ggml_opt_default_params(GGML_OPT_LBFGS);
        ...
        if (ex % 64 == 0) {
            sample_softmax_batch(ctx0, logits, after_opt_probs,
                                 after_opt_best_samples);
...

    {
        ...
        struct ggml_tensor *tokens_input =
            ggml_new_tensor_1d(model.ctx, GGML_TYPE_I32, n_tokens);
        struct ggml_tensor *targets =
            ggml_new_tensor_2d(model.ctx, GGML_TYPE_F32, n_vocab, n_tokens);
        ...
        for (int i = 0; i < n_gen; ++i) {
            ...
            int n_past = 0;
            struct ggml_tensor *logits = forward(
                &model, &kv_self, ctx0, &gf, tokens_input, sample_ctx, n_past);

            ggml_build_forward_expand(&gf, logits);
            ggml_graph_compute_helper(work_buffer, &gf, /*n_threads*/ 1);

            struct ggml_tensor *best_samples =
                ggml_new_tensor_1d(ctx0, GGML_TYPE_I32, sample_ctx);
            struct ggml_tensor *probs =
                ggml_new_tensor_2d(ctx0, GGML_TYPE_F32, n_vocab, sample_ctx);
