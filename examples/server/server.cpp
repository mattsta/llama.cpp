...
struct llama_client_slot {
    ...
    void reset() {
        ...
        images.clear();
        // llama_set_rng_seed(ctx, params.seed); in batched the seed
        // matter???????
    }
...
struct llama_server_context {
    ...
    bool load_model(const gpt_params &params_) {
        ...
        if (!params.mmproj.empty()) {
            ...
            if (clp_ctx == nullptr) {
                LOG_ERROR("unable to load clip model",
                          {{"model", params.mmproj}});
                return false;
            }
        ...
        if (multimodal) {
            ...
            if (n_embd_clip != n_embd_llm) {
                LOG_TEE("%s: embedding dim of the multimodal projector (%d) is "
                        "not equal to that of LLaMA (%d). Make sure that you "
    ...
    bool launch_slot_with_data(llama_client_slot *&slot, json data) {
        ...
        slot->sparams.temp =
            json_value(data, "temperature", default_sparams.temp);
        ...
        slot->sparams.penalty_present = json_value(
            data, "presence_penalty", default_sparams.penalty_present);
        ...
        if (multimodal) {
            ...
            if (images_data != data.end() && images_data->is_array()) {
                for (const auto &img : *images_data) {
                    ...
                    slot_image img_sl;
                    img_sl.id = img.count("id") != 0 ? img["id"].get<int>()
                                                     : slot->images.size();
                    int width, height, channels;
                    std::vector<uint8_t> image_buffer = base64_decode(data_b64);
                    data_b64.clear();
                    auto data = stbi_load_from_memory(
                        image_buffer.data(), image_buffer.size(), &width,
                        &height, &channels, 3);
                    if (!data) {
                    ...
                    LOG_TEE("slot %i - image loaded [id: %i] resolution (%i x "
                            "%i)\n",
                ...
                // process prompt
                // example: system prompt [img-102] user [img-103] describe
                // [img-134] -> [{id: 102, prefix: 'system prompt '}, {id: 103,
                // prefix: ' user '}, {id: 134, prefix: ' describe '}]}
                if (slot->images.size() > 0 && !slot->prompt.is_array()) {
                    std::string prompt = slot->prompt.get<std::string>();
                    size_t pos = 0, begin_prefix = 0;
                    std::string pattern = "[img-";
                    while ((pos = prompt.find(pattern, pos)) !=
                           std::string::npos) {
                        ...
                        if (end_pos != std::string::npos) {
                            ...
                            try {
                                ...
                                for (slot_image &img : slot->images) {
                                    if (img.id == img_id) {
                                        found = true;
                                        img.prefix_prompt = prompt.substr(
                                            begin_prefix,
                                ...
                                if (!found) {
                                    LOG_TEE("ERROR: Image with id: %i, not "
                                            "found.\n",
                                            img_id);
                                    slot->images.clear();
                                    return false;
    ...
    bool process_token(completion_token_output &result,
                       llama_client_slot &slot) {
        ...
        if (slot.multibyte_pending == 0) {
            ...
            size_t stop_pos = find_stopping_strings(str_test, token_str.size(),
                                                    STOP_FULL, slot);
            if (stop_pos != std::string::npos) {
                is_stop_full = true;
                slot.generated_text.erase(slot.generated_text.begin() + pos +
                                              stop_pos,
        ...
        // check the limits
        if (slot.n_decoded > 2 && slot.has_next_token &&
            !slot.has_budget(params)) {
        ...
        LOG_VERBOSE("next token",
                    {
                        {"token", result.tok},
                        {"token_text",
                         tokens_to_output_formatted_string(ctx, result.tok)},
    ...
    json get_formated_generation(llama_client_slot &slot) {
        const auto eos_bias =
            slot.sparams.logit_bias.find(llama_token_eos(model));
    ...
    bool ingest_images(llama_client_slot &slot, int n_batch) {
        ...
        while (image_idx < (int)slot.images.size()) {
            ...
            for (int i = 0; i < (int)append_tokens.size(); ++i) {
                llama_batch_add(batch, append_tokens[i], slot.n_past, {slot.id},
                                true);
                slot.n_past += 1;
            }
        }
    ...
    bool update_slots() {
        ...
        if (all_slots_are_idle) {
            if (system_prompt.empty() && clean_kv_cache) {
                LOG_TEE("all slots are idle and system prompt is empty, clear "
                        "the KV cache\n");
                kv_cache_clear();
            }
        ...
        for (llama_client_slot &slot : slots) {
            if (slot.is_processing() &&
                ...
                LOG_TEE("slot %d: context shift - n_keep = %d, n_left = %d, "
                        "n_discard = %d\n",
        ...
        for (auto &slot : slots) {
            ...
            llama_batch_add(batch, slot.sampled,
        ...
        if (params.cont_batching || batch.n_tokens == 0) {
            for (auto &slot : slots) {
                const bool has_prompt =
                    slot.prompt.is_array() ||
                    (slot.prompt.is_string() &&
                     !slot.prompt.get<std::string>().empty()) ||
                    !slot.images.empty();

                // empty prompt passed -> release the slot and send empty
                // response
                ...
                if (slot.state == IDLE && slot.command == LOAD_PROMPT) {
                    ...
                    if (slot.infill) {
                        ...
                        auto prefix_tokens =
                            tokenize(slot.params.input_prefix, false);
                        auto suffix_tokens =
                            tokenize(slot.params.input_suffix, false);
                        ...
                        prompt_tokens = tokenize(
                            slot.prompt,
                            system_prompt.empty()); // add BOS if there isn't
                                                    // system prompt
                    }
                    ...
                    if (!slot.params.cache_prompt) {
                        ...
                        slot.params.n_keep =
                            std::min(slot.n_ctx - 4, slot.params.n_keep);
                        ...
                        if (slot.num_prompt_tokens >= slot.n_ctx) {
                            ...
                            const int erased_blocks =
                                (slot.num_prompt_tokens - slot.params.n_keep -
                                 n_block_size) /
                                n_block_size;
                        ...
                        // push the prompt into the sampling context (do not
                        // apply grammar)
                        for (auto &token : prompt_tokens) {
                            llama_sampling_accept(slot.ctx_sampling, ctx, token,
                                                  false);
                        }

                        slot.n_past =
                            common_part(slot.cache_tokens, prompt_tokens);
                        slot.num_prompt_tokens_processed =
                            slot.num_prompt_tokens - slot.n_past;

                        LOG_TEE("slot %d : in cache: %i tokens | to process: "
                                "%i tokens\n",
                                slot.id, slot.n_past,
                                slot.num_prompt_tokens_processed);
                    }
                    ...
                    llama_kv_cache_seq_rm(
                        ctx, slot.id, system_tokens.size() + slot.n_past, -1);
                    ...
                    if (slot.n_past == slot.num_prompt_tokens) {
                        ...
                        // logits.
                        LOG_TEE("slot %d : we have to evaluate at least 1 "
                    ...
                    std::vector<llama_token> prefix_tokens =
                        has_images
                            ? tokenize(slot.images[0].prefix_prompt, true)
                            : prompt_tokens;
                    for (; slot.n_past < (int)prefix_tokens.size();
                         ++slot.n_past) {
        ...
        for (int32_t i = 0; i < (int32_t)batch.n_tokens; i += n_batch) {
            ...
            if (ret != 0) {
                if (n_batch == 1 || ret < 0) {
                    ...
                    LOG_TEE("%s : failed to decode the batch, n_batch = %d, "
                            "ret = %d\n",
                            __func__, n_batch, ret);
                    return false;
                }

                LOG_TEE("%s : failed to find free space in the KV cache, "
                        "retrying with smaller n_batch = %d\n",
                        __func__, n_batch / 2);
            ...
            for (auto &slot : slots) {
                ...
                const llama_token id = llama_sampling_sample(
                    slot.ctx_sampling, ctx, NULL, slot.i_batch - i);

                llama_sampling_accept(slot.ctx_sampling, ctx, id, true);

                if (slot.n_decoded == 1) {
                    slot.t_start_genereration = ggml_time_us();
                    slot.t_prompt_processing = (slot.t_start_genereration -
                ...
                for (size_t i = 0; i < std::min(cur_p.size, (size_t)n_probs);
                     ++i) {
...
static void server_print_usage(const char *argv0, const gpt_params &params,
    ...
    printf("  -c N,  --ctx-size N       size of the prompt context (default: "
           "%d)\n",
           params.n_ctx);
    printf("  --rope-freq-base N        RoPE base frequency (default: loaded "
           "from model)\n");
    printf("  --rope-freq-scale N       RoPE frequency scaling factor "
    ...
    printf("                            not recommended: doubles context "
    ...
    printf("  -mg i, --main-gpu i   the GPU to use for scratch and small "
           "tensors\n");
    printf("  -nommq, --no-mul-mat-q\n");
    printf("                        use cuBLAS instead of custom mul_mat_q "
           "CUDA kernels.\n");
    ...
    printf("                        set an alias for the model, will be added "
           "as `model` field in completion response\n");
...
static void server_params_parse(int argc, char **argv, server_params &sparams,
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "--port") {
            ...
            for (size_t i_device = 0; i_device < LLAMA_MAX_DEVICES;
                 ++i_device) {
...
int main(int argc, char **argv) {
    ...
    svr.Get("/", [](const httplib::Request &, httplib::Response &res) {
        res.set_content(reinterpret_cast<const char *>(&index_html),
                        index_html_len, "text/html");
        return false;
    });
    ...
    svr.Get("/json-schema-to-grammar.mjs",
            [](const httplib::Request &, httplib::Response &res) {
                res.set_content(
                    reinterpret_cast<const char *>(&json_schema_to_grammar_mjs),
    ...
    svr.Get("/props",
            [&llama](const httplib::Request & /*req*/, httplib::Response &res) {
                ...
                json data = {{"user_name", llama.name_user.c_str()},
                             {"assistant_name", llama.name_assistant.c_str()}};
                res.set_content(data.dump(), "application/json");
            });

    svr.Post("/completion", [&llama](const httplib::Request &req,
        ...
        if (!json_value(data, "stream", false)) {
            ...
            if (!result.error && result.stop) {
                res.set_content(
                    result.result_json.dump(-1, ' ', false,
            ...
            const auto chunked_content_provider =
                [task_id, &llama](size_t, httplib::DataSink &sink) {
                    while (true) {
                        task_result result = llama.next_result(task_id);
                        if (!result.error) {
                            const std::string str =
    ...
    svr.Post("/infill", [&llama](const httplib::Request &req,
                                 httplib::Response &res) {
        json data = json::parse(req.body);
    ...
    svr.Get("/model.json",
            [&llama](const httplib::Request &, httplib::Response &res) {
    ...
    svr.Post("/detokenize", [&llama](const httplib::Request &req,
                                     httplib::Response &res) {
        const json body = json::parse(req.body);
        std::string content;
    ...
    svr.Post("/embedding", [&llama](const httplib::Request &req,
                                    httplib::Response &res) {
        const json body = json::parse(req.body);
        json prompt;
        if (body.count("content") != 0) {
        ...
        const int task_id = llama.request_completion(
            {{"prompt", prompt}, {"n_predict", 0}}, false);
        task_result result = llama.next_result(task_id);
    ...
    // GG: if I put the main loop inside a thread, it crashes on the first
    // request when build in Debug!?
