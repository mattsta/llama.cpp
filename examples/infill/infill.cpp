...
int main(int argc, char **argv) {
    ...
    if (params.logits_all) {
        printf("\n************\n");
        printf("%s: please use the 'perplexity' tool for perplexity "
               "calculations\n",
               __func__);
        printf("************\n\n");
    ...
    if (!params.interactive_first &&
        ...
        printf("\n************\n");
        printf("%s: please use '--interactive_first' or specify '--in_prefix' "
               "and/or '--in_suffix'\n",
               __func__);
        printf("************\n\n");
    ...
    if (suff_rm_leading_spc && params.input_suffix.find_first_of(" ") == 0 &&
    ...
    std::vector<llama_token> inp_pfx =
        ::llama_tokenize(ctx, params.input_prefix, false);
    std::vector<llama_token> inp_sfx =
        ::llama_tokenize(ctx, params.input_suffix, false);
    ...
    if (ctx_guidance) {
        ...
        guidance_inp = ::llama_tokenize(ctx_guidance,
                                        sparams.cfg_negative_prompt, add_bos);
    ...
    while (n_remain != 0 || params.interactive) {
        ...
        if (!embd.empty()) {
            ...
            if (n_past + (int)embd.size() + std::max<int>(0, guidance_offset) >
                ...
                LOG("context full, swapping: n_past = %d, n_left = %d, n_ctx = "
                    "%d, n_keep = %d, n_discard = %d\n",
                    n_past, n_left, n_ctx, params.n_keep, n_discard);

                llama_kv_cache_seq_rm(ctx, 0, params.n_keep + 1,
                                      params.n_keep + n_discard + 1);
                llama_kv_cache_seq_shift(ctx, 0, params.n_keep + 1 + n_discard,
                                         n_past, -n_discard);
        ...
        if ((int)embd_inp.size() <= n_consumed) {
            ...
            if ((llama_sampling_last(ctx_sampling) == llama_token_eot(model) ||
                 is_interacting) &&
                params.interactive) {
                if (is_interacting && !params.interactive_first) {
                    // print an eot token
                    printf("%s",
                           llama_token_to_piece(ctx, llama_token_eot(model))
                               .c_str());
                }
                fflush(stdout);
                printf("\n");
                ...
                do {
                    another_line =
                        console::readline(line, params.multiline_input);
                    buffer += line;
                } while (another_line);
                ...
                if (params.escape) {
                    // process escape sequences, for the initial prompt this is
                    // done in common.cpp when we load the params, but for the
                    // interactive mode we need to do it here
                    process_escapes(params.input_prefix);
                    process_escapes(params.input_suffix);
                }
                suff_rm_leading_spc = params.escape;
                if (suff_rm_leading_spc &&
                    params.input_suffix.find_first_of(' ') == 0 &&
                    params.input_suffix.size() > 1) {
                    params.input_suffix.erase(0, 1);
                    suff_rm_leading_spc = false;
                }
            ...
            else if (llama_sampling_last(ctx_sampling) ==
            ...
            if (n_past > 0 && is_interacting && !params.interactive) {
                ...
                if (buffer.length() > 1) {
                    ...
                    embd_inp.insert(embd_inp.end(), line_inp.begin(),
                                    line_inp.end());
