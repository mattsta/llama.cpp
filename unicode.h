...
static uint32_t codepoint_from_utf8(const std::string &utf8, size_t &offset) {
    ...
    if (!(utf8[offset + 0] & 0x80)) {
        ...
        auto result =
            ((utf8[offset + 0] & 0x1f) << 6) | (utf8[offset + 1] & 0x3f);
        offset += 2;
        return result;
...
codepoints_to_utf16(const std::vector<uint32_t> &cps) {
    std::vector<uint16_t> result;
    for (size_t i = 0; i < cps.size(); ++i) {
...
static uint32_t codepoint_from_utf16(const std::vector<uint16_t> &utf16,
    ...
    if (((utf16[0] >> 10) << 10) != 0xd800) {
        ...
        auto result =
            0x10000 + (((utf16[0] & 0x03ff) << 10) | (utf16[1] & 0x03ff));
        offset += 2;
        return result;
    }
    throw std::invalid_argument("invalid string");
...
codepoints_from_utf16(const std::vector<uint16_t> &utf16) {
    std::vector<uint32_t> result;
    size_t offset = 0;
